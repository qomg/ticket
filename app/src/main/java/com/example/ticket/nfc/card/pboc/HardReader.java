...
import com.example.ticket.nfc.tech.Iso7816.BerT;
...
final class HardReader extends PbocCard {
	...
	private HardReader(Iso7816.Tag tag, byte[] name) {
		super(tag);
		this.name = (name != null) ? Util.toHexString(name, 0, name.length) : "未知";
	}

	@SuppressWarnings("unchecked")
	final static HardReader load(Iso7816.Tag tag) {
		...
		// build result string
		/*--------------------------------------------------------------*/
		final HardReader ret = new HardReader(tag, name);
